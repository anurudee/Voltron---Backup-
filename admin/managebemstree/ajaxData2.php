...
if(isset($_POST["base_id"]) && !empty($_POST["base_id"])){
    ...
    if($rowCount > 0){
        ...
        echo '<option value="">Sub Tree Unavailable</option>';
    }
}
...
if(isset($_POST["sub6_id"]) && !empty($_POST["sub6_id"])){
    //Get all sub6 data
    $query = $db->query("SELECT * FROM sub7 WHERE sub6_id = ".$_POST['sub6_id']." AND status = 1");
...
if(isset($_POST["sub7_id"]) && !empty($_POST["sub7_id"])){
    //Get all sub7 data
    $query = $db->query("SELECT * FROM sub8 WHERE sub7_id = ".$_POST['sub7_id']." AND status = 1");
